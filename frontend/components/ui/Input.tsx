      ...
      <div className="w-full space-y-2">
        ...
        <div className="relative">
          ...
          <input
            id={inputId}
            ref={ref}
            className={`flex h-11 w-full rounded-xl border border-teal-200 bg-white px-3 py-2 text-sm ring-offset-white file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-teal-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 disabled:cursor-not-allowed disabled:opacity-50 dark:border-teal-800 dark:bg-teal-950 dark:text-white dark:ring-offset-teal-950 dark:placeholder:text-teal-600 transition-colors ${icon ? 'pl-10' : ''} ${error ? 'border-red-500 focus-visible:ring-red-500' : ''} ${className}`}
            {...props}
          />
        </div>
        {error && (
